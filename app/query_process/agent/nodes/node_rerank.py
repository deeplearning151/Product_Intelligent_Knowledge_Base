...
def step3_topk_and_gap(rerank_score_list):
    """
    对rerank模型打分以后得有序集合进行再次算法筛选！
    取出动态的topk元素即可
    问题稠密和稀疏向量 = （问题进行向量混合搜索） =》 rrf(同源的排序 rank + weight ) => rrf
    问题+假设性答案稠密和稀疏向量 = （问题+假设性答案进行向量混合搜索） => rerank => 算法 =》 topk => mcp
...
def node_rerank(state):
    ...
    """
    doc_list = step1_merge_rrf_mcp(state)
    # 2. 启用rerank进行精排 （数据和分）
    """
    [
      {
    ...
    """
    rerank_score_list = step2_rerank_doc_list(doc_list, state)
    # 3. 启动算法进行防断崖以及topk处理  0.9  0.89  0.35
    final_doc_list = step3_topk_and_gap(rerank_score_list)
    # 4. 结果装到state中即可
