...
def step3_create_answer(state, prompt):
    """
    使用模型生成最终的答案
    :param state:
    :param prompt:
...
def step4_extract_images_url(state):
    """
...
def node_answer_output(state):
    ...
    answer_exists = step1_check_answer(state)
    if not answer_exists:
        # 2. 没有 成对应的润色的提示词 prompt
        prompt = step2_load_prompt(state)
        # 3. 没有 使用模型润色答案 -》 结果 -> 文本
        answer = step3_create_answer(state, prompt)
        # 4. 没有 提取原来topklist中的图片地址，单独返回【see】
        images_url = step4_extract_images_url(state)
        # 6. sse-final->返回图片
        if images_url:
            # 不管流 和 非流都需要返回图片
