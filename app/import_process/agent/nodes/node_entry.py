...
def node_entry(state: ImportGraphState) -> ImportGraphState:
    ...
    涉及的state: local_file_path is_read_md_enabled is_read_pdf_enabled md_path pdf_path file_title
    未来要实现:
    1. 进入节点的日志输出，【节点 + 核心参数】
        记录任务状态【哪个任务开始了】 -> 给前端推送信息（埋点）
    2. 参数校验（local_file_path -> 没有传入文件 -> end / local_dir -> 没有传入输出文件 -> 创建一个临时）
    3. 解析文件类型，修改state对应的参数local_file_path -> md | pdf
    ...
    # 提取 file_title /xx/xxx/aaa.pdf -> aaa -> 为了后期大模型没有识别出来当前文件对应item_name -> file_title进行兜底
