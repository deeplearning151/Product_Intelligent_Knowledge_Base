...
def step2_prepare_collections(state):
    """
    创建chunks对应的集合！！！
    :param state:
    :return:
...
def step3_delete_old_data(milvus_client, item_name):
    """
    删除旧数据 根据item_name删除
    :param milvus_client:
    :param item_name:
...
def step4_insert_collections(milvus_client,chunks):
    """
    插入集合的数据！
    :param chunks:
    :return:  chunks -> 主键回显
...
def node_import_milvus(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        milvus_client = step2_prepare_collections(state)
        # 3. 删除旧数据
        step3_delete_old_data(milvus_client, chunks[0]['item_name'])
        # 4. 插入chunks的数据即可
        with_id_chunks = step4_insert_collections(milvus_client,chunks)
